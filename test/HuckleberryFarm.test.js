...
contract('HuckleberryFarm', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.finn = await FINN.new({ from: alice });
    });

    it.only('should set correct state variables', async () => {
        this.farm = await HuckleberryFarm.new(this.finn.address, dev, '1000', (await time.latest())+10, (await time.latest()) + 110, { from: alice });
        await this.finn.transferOwnership(this.farm.address, { from: alice });
        const finn = await this.farm.finn();
        const devaddr = await this.farm.devaddr();
        const owner = await this.finn.owner();
        assert.equal(finn.valueOf(), this.finn.address);
        assert.equal(devaddr.valueOf(), dev);
        assert.equal(owner.valueOf(), this.farm.address);
    });

    it('should allow dev and only dev to update dev', async () => {
        this.farm = await HuckleberryFarm.new(this.finn.address, dev, '1000', '0', '100', '1000', '10000', { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '100', '100', '1000', '10000', { from: alice });
        ...
        it('should give out WASPs only after farming time', async () => {
            ...
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '0');
            ...
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '500');
            await time.advanceBlockTo('104');
            await this.farm.deposit(0, '0', { from: bob }); // block 105
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '2500');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '125');
            assert.equal((await this.finn.totalSupply()).valueOf(), '2625');
        });

        it('should not distribute WASPs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '200', '200', '1000', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.farm.add('100', this.lp.address, true);
            await this.lp.approve(this.farm.address, '1000', { from: bob });
            await time.advanceBlockTo('199');
            assert.equal((await this.finn.totalSupply()).valueOf(), '0');
            ...
            assert.equal((await this.finn.totalSupply()).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '0');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '990');
            await time.advanceBlockTo('219');
            await this.farm.withdraw(0, '10', { from: bob }); // block 220
            assert.equal((await this.finn.totalSupply()).valueOf(), '5250');
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '5000');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '250');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '1000');
        });

        it('should distribute WASPs properly for each staker', async () => {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '300', '300', '1000', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.farm.add('100', this.lp.address, true);
            ...
            //   HuckleberryFarm should have the remaining: 5250 - 250 - 2833 = 2417
            await time.advanceBlockTo('319')
            await this.farm.deposit(0, '10', { from: alice });
            assert.equal((await this.finn.totalSupply()).valueOf(), '5250');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '2833');
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(this.farm.address)).valueOf(), '2167');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '250');
            // Bob withdraws 5 LPs at block 330. At this point:
            //   Bob should have: 4*5*2/3*100 + 2*5*2/6*100 + 10*5*2/7*100 = 3095
            await time.advanceBlockTo('329')
            await this.farm.withdraw(0, '5', { from: bob });
            assert.equal((await this.finn.totalSupply()).valueOf(), '10500');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '2833');
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '3095');
            assert.equal((await this.finn.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(this.farm.address)).valueOf(), '4072');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '500');
            ...
            assert.equal((await this.finn.totalSupply()).valueOf(), '26250');
            assert.equal((await this.finn.balanceOf(dev)).valueOf(), '1250');
            // Alice should have: 2833 + 10*5*2/7*100 + 10*5*2/6.5*100 = 5800
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '5800');
            // Bob should have: 3095 + 10*5*1.5/6.5*100 + 10*5*1.5/4.5*100 = 5915
            assert.equal((await this.finn.balanceOf(bob)).valueOf(), '5915');
            // Carol should have: 2*5*3/6*100 + 10*5*3/7*100 + 10*5*3/6.5*100 + 10*5*3/4.5*100 + 10*5*100 = 13284
            assert.equal((await this.finn.balanceOf(carol)).valueOf(), '13284');
        ...
        it('should give proper WASPs allocation to each pool', async () => {
            ...
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '400', '400', '1000', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '1000', { from: alice });
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '5000');
            // Bob deposits 10 LP2s at block 425
            await time.advanceBlockTo('424');
            await this.farm.deposit(1, '5', { from: bob });
            // Alice should have 5000 + 5*5*1/3*100 = 5833 pending reward
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '5833');
            await time.advanceBlockTo('430');
            // At block 430. Bob should get 5*5*2/3*100 = 1666.
            // At block 430. Alice should get 5833 + 5*5*1/3*100 = 6666.
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '6666');
            assert.equal((await this.farm.pendingReward(1, bob)).valueOf(), '1666');
        });

        it('should stop giving bonus WASPs after the bonus period ends', async () => {
            // 100 per block farming rate starting at block 500 with bonus until block 600
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '500', '500', '600', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '1000', { from: alice });
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '5500');
            // At block 606, Alice withdraws all pending rewards and should get 5600.
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '5600');
        });

        it('state test 1', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '600', '800', '1000', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '15000');

            // At block 811, Alice withdraws all pending rewards and should get 15500.
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '15500');
        });

        it('state test 2', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '900', '910', '920', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '100000', { from: alice });
            await this.farm.add('1', this.lp.address, true);
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '1000');

            // At block 811, Alice withdraws all pending rewards and should get 21000.
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '1100');
        });

        it('state test 3', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '1000', '1010', '1020', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '100000', { from: alice });
            await this.farm.add('1', this.lp.address, true);
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '2500');

            // Alice withdraws all pending rewards and should get 2600.
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '2600');
        });

        it('state test 4', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '1100', '1110', '1120', '10000', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '100000', { from: alice });
            await this.farm.add('1', this.lp.address, true);
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '6500');

            // Alice withdraws all pending rewards and should get 6600.
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '6600');
        });

        it('state test 5', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '1135', '1140', '1150', '1200', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '100000', { from: alice });
            await this.farm.add('1', this.lp.address, true);
            ...
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '0');
        });

        it('state test 6', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '1300', '1310', '1320', '1400', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            await this.lp.approve(this.farm.address, '100000', { from: alice });
            await this.farm.add('1', this.lp.address, true);
            ...
            await this.farm.deposit(0, '0', { from: alice });
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '0');
            assert.equal((await this.finn.balanceOf(alice)).valueOf(), '1000');
        });

        it('multi-decimal test', async () => {
            this.farm = await HuckleberryFarm.new(this.finn.address, dev, '100', '1500', '1500', '1600', '1700', { from: alice });
            await this.finn.transferOwnership(this.farm.address, { from: alice });
            ...
            assert.equal((await this.farm.pendingReward(0, alice)).valueOf(), '12500');
            assert.equal((await this.farm.pendingReward(1, bob)).valueOf(), '12375');
            assert.equal((await this.farm.pendingReward(2, carol)).valueOf(), '6187');
            assert.equal((await this.farm.pendingReward(2, dev)).valueOf(), '6062');
        });
    });
});
